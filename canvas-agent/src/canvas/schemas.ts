...
const generationOptionsSchema = z.object({
    ...
    model3dTexture: z.string().optional(),
    model3dPbr: z.string().optional(),
    model3dTextureQuality: z.string().optional(),
    model3dFaceLimit: z.string().optional(),
    model3dQuad: z.string().optional(),
});
...
export const toolInputSchemas = {
    ...
    canvas_generate_model3d: generationFlowSchema.merge(generationOptionsSchema),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    canvas_create_node: "创建任意类型节点：text、image、config、video、audio、model3d。适合创建占位图、媒体占位、3D 占位、配置节点或自定义 metadata 节点。",
    ...
    canvas_create_generation_flow: "创建通用生成流程：提示词文本节点、生成配置节点、参考节点连线，可用于文案、生图、视频、音频或 3D。",
    ...
    canvas_generate_model3d: "创建 3D 生成流程并立即触发生成。仅有 prompt 走文生 3D；把已有图片节点 ID 放入 referenceNodeIds 则走图生 3D（1 张）或多视图生 3D（2–4 张）。可选 model、model3dTexture、model3dPbr、model3dTextureQuality、model3dFaceLimit、model3dQuad，未指定时沿用画布当前 3D 设置。",
