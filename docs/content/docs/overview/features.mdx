...
- API keys remain in browser storage. Requests go directly to configured providers by default; a Docker deployment forwards them through the site's same-origin `/proxy` so the browser is not blocked by CORS, without storing the keys on the server.
