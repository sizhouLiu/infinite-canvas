...
+ [新增] 画布接入 Tripo 3D 生成：新增 3D 节点和「3D」生成模式，支持文生 3D 与图生 3D，生成结果存到本地并提供可拖拽旋转的 3D 查看器。
+ [新增] 渠道新增 Tripo 规范和「3D」能力，配套 3D 设置（贴图、PBR、贴图质量、面数上限）。
+ [新增] Tripo 图片生成：文生图与图生图接入 Tripo 渠道，按模型自动适配尺寸、质量和背景参数。
+ [新增] Tripo 双区域：内置国内版（`.com`）和海外版（`.ai`）两个渠道，渠道编辑器可切换区域，两边各自保存 API Key。
+ [修复] 未开启本地代理时使用 Tripo 生成，改为直接提示开启本地代理，不再只在控制台报一堆 CORS 错误。
+ [新增] Tripo 多视图：2–4 张上游图片走多视图生成 3D；图片节点新增「多视图」工具，可由单图生成四视图，并对已有四视图按视图分别改图。
+ [新增] 3D 节点新增「3D 操作」菜单：重新贴图、自动绑骨、套用动画、减面、分割部件、网格补全、格式转换（FBX / GLTF / USDZ / OBJ / STL / 3MF）。
+ [新增] 自动绑骨前先跑免费的可绑骨检测，模型不可绑骨时直接提示，不再白花 credits。
+ [修复] 3D 节点连接多张上游图片时只用第一张、其余被静默丢弃；现在改走多视图，超过 4 张会明确提示。
+ [修复] 3D 生成连接图片后不再强制输入提示词，图生 3D 和多视图生 3D 可直接生成；3D 提示词输入框补上了缺失的占位文案。
+ [调整] 3D 节点连上图片后直接隐藏提示词输入框，改为说明将走图生 3D 还是多视图生 3D（单张图接到左侧对应视角接点，打组的图在组内标记正/左/后/右，未指定的按连线顺序补上剩下的视图；超过 4 张会说明只用前 4 张），不再留一个输入了也会被丢掉的框。
+ [新增] 3D 设置新增「拓扑」选项，可选三角面或四边面；仅在支持的模型（P2、v3.0 以上）上显示。
+ [修复] 图生 3D 和多视图生 3D 上传参考图时提示未配置 API Key（实际已配置），现在上传前也会先解析模型所属渠道。
+ [优化] Tripo 请求失败改为报可读错误：401 会直接说明国内版和海外版是两个独立账号、Key 不通用，代理连不上时也会明确提示，不再只抛出 Request failed with status code 401。
+ [新增] 双击 3D 节点可在大窗口里查看模型，窗口内直接可拖拽旋转和缩放。
+ [新增] 大窗口 3D 查看器新增实体 / 白模 / 白模+线框 / 线框 / 骨骼五种查看模式，带骨骼的模型可显示骨架并播放、切换自带动画，另有相机复位。
+ [新增] 3D 查看器支持加载 FBX，H 系列四边面产物和绑骨、套动画选 fbx 输出的模型现在能直接在画布和大窗口里预览，不再只能下载。
+ [新增] 四边面模型的线框模式改为显示四边面布线，按需还原被三角化切开的对角线，不再把每条三角边都画出来。
+ [修复] 3D 产物统一按文件实际格式记录类型，不再一律记成 glb，导致 FBX 产物无法识别。
+ [新增] 内置四视图生成：图片节点新增「四视图」工具，用当前配置的图片模型分别生成正/左/后/右四个视角，不依赖 Tripo 渠道和 credits；四个视角逐个串行请求（生图并发通常只有 1），撞到并发上限会按接口给的 `Retry-After` 等待后重试该视角，单个视角失败会提示并保留其余视角。
+ [新增] 模型拆件：3D 节点的「分割部件」新增「拆成独立部件节点」开关（默认开），把 Tripo 分割产出的整体 glb 按部件名（脸、头发、身体等）在本地拆成多个 glb，每个部件生成独立 3D 节点并从分割节点连线，可单独预览、下载和继续操作。
+ [调整] 图片节点的「多视图」工具改为默认显示，不用再去快捷工具设置里勾选。
+ [修复] 生图接口报错不再丢掉限流信息：原来一律包成纯文本 Error，导致 HTTP 状态码、`Retry-After` 头和 Tripo 的 `code: 2000` 全部丢失，调用方无法据此退避重试。
+ [修复] 多视图生 3D 报「invalid model 'xxx::P2-20260801'」：模型选择器存的是「渠道ID::模型名」，多视图请求把这个复合值原样发给了 Tripo，现在所有 3D 操作请求都统一剥掉渠道前缀再发。
+ [修复] 生成失败后点重试报「找不到提示词」无法重试：图生 3D、多视图生 3D、多参考图改图本来就不需要提示词，重试却一律要求有提示词，导致失败节点没有任何恢复途径；现在重试的提示词要求和生成时一致——文本和音频始终需要，图片和 3D 只在没有参考图时需要。
+ [修复] 3D 节点生成失败后点重试会走成生图请求，把图片结果写进 3D 节点；现在改走 3D 生成流程，结果写回原节点。
+ [修复] 多视图生 3D 会出现两个相同视角（例如两个正面）导致模型诡异：视图归属原来是逐张独立判断的，多视图产出的图片沿用自带视角、其余图片按连线下标取视角，两条规则互不知情，撞车时既可能重复视角也可能空缺正面视角（正面视角缺失会直接被接口拒绝）；现在改为对整组图片统一分配，自带视角优先占位，其余图片只填没被占用的视角。
+ [新增] 多视图生成出来的四张图自动打组：四视图落到画布后会被一个组节点包住，可整体拖动、整体连线（把组连给 3D 节点等于把四个视角一起送进去），也可随时解散组。只成功一个视角时不打组（和手动打组一样至少要两个成员）。
+ [新增] 生成历史：左侧面板新增「历史」页签，按画布记录每次生成的任务 ID、提示词、模型和结果路径（图片 / 视频 / 音频 / 3D / 多视图 / 3D 操作 / 局部编辑 / 多视角 / 重试均计入），可搜索、按类型筛选、定位到节点、复制任务 ID；请求发出前就建记录，生成中刷新或关页也能保住任务 ID。历史只存元数据、不锁住结果文件：节点删除后对应文件仍会被存储回收清掉，记录用于回查任务和重新生成；删除画布时该画布的记录一并清除。
+ [新增] 参数配置改为 ComfyUI 式的参数行：生成参数直接摊开在节点下方面板里，五种模式（生图 / 视频 / 音频 / 3D / 文本）共用同一套行式面板，不用再一项一项点开气泡；参数区可折叠。
+ [新增] 配置节点可作为参数节点：从配置节点连线到生成节点，该节点的参数就由配置节点提供，一个配置节点可同时驱动多个下游节点；下游节点的参数行显示为只读并标出来源，可一键定位或断开。
+ [变更] 配置节点连出到生成节点后转为参数节点：不再显示自己的生成按钮，下方面板也从「组装提示词」切换为参数行；未连出的配置节点行为不变。
+ [调整] 连线新增参数标记：只有手动从配置节点拉出的连线才算参数连线，生成时自动连到结果节点的那根线不算，因此已有画布里的配置节点行为不受影响；参数连线也不参与生成输入和失败重试的来源查找。
+ [调整] 3D 查看器默认相机改为沿 X 轴平视模型（原先是右前上方的 3/4 俯视角），相机复位也回到这个机位。
+ [新增] 3D 查看器支持拖动移动模型：右键拖动（触控板双指拖动）可平移模型，左键拖动仍是旋转，滚轮缩放；「相机复位」会一并复位平移。查看器内右键不再弹出节点右键菜单。
+ [调整] 3D 设置的「面数上限」从数字输入框改为滑块（48–25000，步长 100），默认 5000，旁边保留数字框可精确输入；另有「自动」开关，开启后不发送 face_limit、由 Tripo 决定面数。升级后原本留空（自动）的设置会迁移为 5000，因此已有画布的 3D 生成面数会被限制到 5000，需要自动的请自行打开开关。
+ [修复] 四边面模型的线框仍然显示三角面：还原对角线时按顶点下标判断边是否被两个三角形共用，而导出器会在 UV 和法线接缝处把同一个位置的顶点拆开，两个三角形因此持有不同的下标，共用边一条都匹配不上，结果一条对角线也没丢掉。现在先按坐标把顶点合并再做拓扑判断。
+ [修复] 记录节点的四边面设置时不再无条件写入所选值：模型不支持四边面（P1、v2.5）时请求本来就不发这个字段，节点上却记成开启，导致查看器按四边面去还原一个三角面产物。
+ [新增] 3D 查看器新增「固有色」查看模式：只画基础色贴图（albedo），不参与光照、不叠加法线和金属度粗糙度等 PBR 贴图，用来看贴图本身而不是光打在上面的效果。
+ [优化] 画布上有多个 3D 节点时不再卡：3D 节点默认只显示一张模型截图，点一下才真正加载模型，同时最多 4 个节点保持真实渲染（超出时最早点开的那个自动退回截图）。原因是每个 3D 节点各占一个 WebGL 上下文和一条不停的渲染循环，浏览器对同时存在的上下文有上限（Chrome 约 8–16 个），二十多个节点会稳定超标。没有预览图的节点（例如本地拆件出来的部件）会在首次加载后自动截一张 720×720 存到本地，之后就用它。
+ [优化] 3D 查看器改为按需渲染：静止的模型只在相机移动、切换查看模式、缩放窗口或播放动画时重绘，不再每秒无条件渲染 60 帧。
+ [新增] 场景拆件：图片节点新增「拆件」工具，先用提示词识别画面里可以单独取出的物件（人物、道具、家具等，最多 12 个），清单可以改名、删除、增补或重新识别，确认后逐个生成带透明背景的单体图，自动打组并从原图连线，之后每个组件都能继续用局部编辑、多角度、重试，或单独接到 3D 节点生成模型。与原有的「切图」不同：切图是按行列切像素，拆件是按画面内容分物件。每个组件一次独立请求、逐个串行（生图并发通常只有 1），撞到并发上限会按接口给的等待时间重试一次，单个组件失败会提示并保留其余组件。当前图片模型不支持透明背景时（Gemini，或非 flare/sunburst 的 Tripo 模型）会在弹窗里提前说明，产出为不透明底。
+ [新增] Docker 镜像内置同源转发代理：浏览器请求经站点 `/proxy` 转到上游，默认只放行 OpenAI / Gemini / Tripo 域名并按 IP 限流，内网地址一律拒绝；API Key 仍留在使用者浏览器里，站点不存储。本机开发行为不变，仍可自行跑 `npx @basketikun/canvas-proxy`。
+ [新增] 画布可将选中的 3D 模型导入本机 Blender：悬停工具栏、右键菜单和多选工具栏提供「导入 Blender」，经 Tripo Bridge 插件的 `ws://127.0.0.1:60600` 发送已存储的模型文件（GLB / GLTF / FBX / OBJ / STL / USDZ）。需在本机 Blender 启用该插件；HTTPS 页面无法连接本机 ws。
+ [新增] 从已有模型的 3D 节点拉线到空白处，创建菜单会列出「3D 操作」（重新贴图、自动绑骨、套用动画、减面、分割部件、网格补全、转换格式），点选后走和悬停菜单相同的参数弹窗；套用动画 / 网格补全在不满足串链条件时置灰。
+ [新增] 双击 3D 节点打开大窗口后，可旋转、平移、缩放到想要的角度，再点「导出当前视角为图片」，在源节点右侧生成一张当前画面的图片节点并连线。
+ [修复] 图片节点点「多视图」报 Tripo `code: 2003`「The image file is empty」：节点里存的是 blob 地址而不是 dataURL，转成文件前没有先把图片读出来，结果上传的是一个 0 字节文件。现在和其他上传路径一样先读成 dataURL 再上传；同时 dataURL 转文件这一步遇到非 dataURL 会直接报「读取图片失败」，不再默默产出空文件、把错误推到接口那一端才暴露。
