...
+ [调整] 3D 节点连上图片后直接隐藏提示词输入框，改为说明将走图生 3D 还是多视图生 3D（多视图产出的图片沿用自带视角，其余图片按连线顺序补上剩下的正/左/后/右视图，超过 4 张会说明只用前 4 张），不再留一个输入了也会被丢掉的框。
...
+ [新增] 多张图片连到 3D 节点时，连线上直接标出正面 / 左侧 / 背面 / 右侧，点标签即可改这张图的视角；改动写入节点，之后的生成沿用（与被占用的视角自动互换，不会丢掉必填的正面）。
