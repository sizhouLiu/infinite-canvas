export default {
    ...
    settingsPanels: {
        ...
        image: { title: "图像设置", quality: "质量", size: "尺寸", align16: "16 倍数对齐", align16Hint: "输入完成后自动向上补成 16 的倍数", resolution: "分辨率", aspectRatio: "宽高比", transparent: "透明背景", transparentHint: "开启后生成无背景的透明图像（仅部分模型可用）", template: "模板", templateHint: "选中后提示词变为可选。", templates: { none: "无", t_pose: "T-pose", character_completion: "角色补全", "3d_enhance": "3D 增强", variants: "设计变体", figure: "全身人物" }, count: "生成张数", images: "{{count}} 张" },
        ...
        model3d: { title: "3D 设置", texture: "贴图", pbr: "PBR 材质", textureQuality: "贴图质量", faceLimit: "面数上限", faceLimitAuto: "自动", topology: "拓扑", topologies: { false: "三角面", true: "四边面" }, quadHint: "四边面仅 P2 与 v3.0 以上模型支持；H 系列（v3.x）开启后产物为 FBX，画布内无法预览，只能下载。面数上限也会变低（例如 P2 为 48–25000）。", onOff: { true: "开启", false: "关闭" }, qualities: { standard: "标准", detailed: "精细", extreme: "极致" } },
        text: { title: "文本设置", reasoning: "推理强度", count: "生成次数" },
        model: { select: "选择模型", assign: "请先在渠道里为{{capability}}指定模型", noMatch: "暂无匹配的{{capability}}模型", addFirst: "请先到配置里添加渠道和模型", capabilities: { image: "生图", video: "视频", text: "文本", audio: "音频", model3d: "3D" } },
    },
    ...
    modelPlugin: {
        ...
        templates: { openai: "OpenAI 规范", gemini: "Gemini 规范", tripo: "Tripo 规范" },
        authoring: {
    ...
    apiErrors: { model3dOpTaskCreateFailed: "3D 操作任务创建失败", model3dOpNoTaskId: "接口没有返回 3D 操作任务 ID", model3dOpTaskQueryFailed: "3D 操作任务状态查询失败", model3dOpFailed: "3D 操作失败", model3dOpTimeout: "3D 操作超时，任务可能仍在服务端进行，可稍后重试", model3dOpSourceMissing: "找不到该节点的模型文件，无法执行该操作", model3dMultiviewFrontRequired: "多视图生成必须包含正面视图", model3dMultiviewTooFew: "多视图生成至少需要 2 张图片", tripoAuthFailed: "Tripo 鉴权失败（401）。国内版（openapi.tripo3d.com）和海外版（openapi.tripo3d.ai）是两个独立账号，一边的 Key 在另一边会直接 401，请确认该渠道的 API Key 和区域匹配", tripoProxyUnreachable: "连不上本地代理，请确认已执行 npx @basketikun/canvas-proxy@latest 且地址端口一致", tripoProxyRequired: "Tripo 接口不支持浏览器直连，请先在「设置 - 本地代理」中开启本地代理并按提示启动代理命令", tripoUploadFailed: "Tripo 参考图上传失败", modelRequired: "请先配置模型", promptRequired: "请先输入提示词", referenceRequired: "请先连接参考图片", tripoImageTaskCreateFailed: "Tripo 生图任务创建失败", tripoImageNoTaskId: "Tripo 接口没有返回生图任务 ID", tripoImageTaskQueryFailed: "Tripo 生图任务状态查询失败", tripoImageGenerationFailed: "Tripo 生图失败", tripoImageTimeout: "Tripo 生图超时，任务可能仍在服务端进行，可稍后重试", tripoImageDownloadFailed: "Tripo 生图结果下载失败，链接可能已过期", requestFailed: "请求失败", requestCanceled: "请求已取消", baseUrlRequired: "请先配置 Base URL", apiKeyRequired: "请先配置 API Key", authenticationFailed: "鉴权失败，请检查 API Key、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "接口地址不存在（404），请检查 Base URL 和模型选择", badGateway: "网关错误（502），接口服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查 Base URL 和 API Key 是否正确", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", audioModelRequired: "请先配置音频模型", model3dModelRequired: "请先配置 3D 模型", model3dPromptRequired: "请先输入 3D 生成提示词", model3dTaskCreateFailed: "3D 生成任务创建失败", model3dTaskQueryFailed: "3D 任务状态查询失败", noModel3dTaskId: "接口没有返回 3D 任务 ID", model3dGenerationFailed: "3D 生成失败", model3dTimeout: "3D 生成超时，任务可能仍在服务端进行，可稍后重试", model3dBanned: "3D 生成被内容策略拒绝，请调整提示词或输入图片", model3dExpired: "3D 任务已过期，请重新生成", model3dDownloadFailed: "3D 模型文件下载失败，链接可能已过期", audioGenerationFailed: "音频生成失败", scriptNoAudio: "模型调用脚本没有返回音频", geminiAudioUnsupported: "Gemini 调用格式暂不支持音频生成，请使用 OpenAI 格式渠道", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口返回了未知格式的数据（字段：{{fields}}），请检查模型或接口兼容性", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", geminiRejected: "Gemini 拒绝了本次请求：{{reason}}", geminiNoImage: "Gemini 接口没有返回图片", maskModelUnsupported: "蒙版编辑暂不支持该模型，请使用其他渠道", noContent: "没有返回内容", modelReadFailed: "读取模型失败", videoTimeout: "{{provider}}视频生成超时，请稍后重试", pluginVideoExpired: "插件视频任务已失效，请重新生成", scriptNoVideo: "模型调用脚本没有返回视频", noPlayableVideo: "视频接口没有返回可播放的视频", noVideoTaskId: "视频接口没有返回任务 ID", videoTaskCreateFailed: "视频任务创建失败", videoGenerationFailed: "视频生成失败", videoTaskQueryFailed: "视频任务查询失败", videoPromptRequired: "请输入视频提示词，或连接参考图片/视频/音频", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", invalidReferenceVideo: "参考视频必须是公网 URL、资产 ID，或本地已保存的视频", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", videoModelRequired: "请先配置视频模型", geminiVideoUnsupported: "Gemini 调用格式暂不支持视频生成，请使用 OpenAI 格式渠道", noVideoTask: "接口没有返回视频任务", videoDownloadFailed: "视频下载失败", localAssetReadFailed: "读取本地资产失败" },
    prompts: {
        title: "提示词中心",
        library: "提示词库",
    ...
    assets: {
        ...
        kinds: { text: "文本", image: "图片", video: "视频", audio: "音频", model3d: "3D 模型" },
        fields: {
            title: "标题",
            titleRequired: "请输入标题",
    ...
    canvas: {
        ...
        model3d: { loading: "正在加载 3D 模型", loadFailed: "3D 模型加载失败", empty: "暂无 3D 模型", generating: "正在生成 3D 模型", clip: "动画", boneCount: "骨骼：{{count}} 根", clipCount: "动画：{{count}} 个片段", rigged: "已绑骨", staticMesh: "静态模型", activate: "点击加载模型", resetCamera: "复位相机", play: "播放动画", pause: "暂停动画", captureView: "导出当前视角为图片", captureTitle: "{{name}} 视角", captured: "已生成图片节点", captureFailed: "无法导出当前视角，请重试", modes: { shaded: "实体", basecolor: "固有色", clay: "白模", claywire: "白模 + 线框", wireframe: "线框", skeleton: "骨骼" } },
        toolbar: {
        ...
        createMenu: {
            fromNode: "引用该节点生成", close: "关闭", text: "文本生成", textDescription: "脚本、广告词、品牌文案", image: "图片生成", video: "视频生成", audio: "音频参考", model3d: "3D 生成", config: "配置节点", configDescription: "模型、尺寸、数量和输入顺序", select: "选择节点",
        },
        node: {
            node: "节点",
        ...
        model3dOps: { title: "3D 操作", run: "开始", editMultiview: "编辑多视图", editMultiviewHint: "只填想修改的视图，留空的视图保持不变。", editMultiviewPlaceholder: "例如：改成红色外套", ops: { texture: "重新贴图", convert: "转换格式", rig: "自动绑骨", retarget: "套用动画", segment: "分割部件", complete: "网格补全", decimate: "减面", multiview: "多视图生成" }, fields: { model: "模型", textureQuality: "贴图质量", pbr: "PBR 材质", format: "目标格式", quad: "四边面", faceLimit: "面数上限", rigType: "骨架类型", spec: "骨骼规范", outFormat: "输出格式", animation: "动画", animateInPlace: "原地播放", granularity: "分割粒度", splitByConnectivity: "按连通性拆分", splitParts: "拆成独立部件节点", completionMode: "补全方式" }, rigTypes: { biped: "双足", quadruped: "四足", hexapod: "六足", octopod: "八足", avian: "鸟类", serpentine: "蛇形", aquatic: "水生" }, granularities: { simple: "粗", balanced: "均衡", detailed: "细" }, completionModes: { ai_completion: "AI 补全", quick_cap: "快速封口" }, views: { front: "正面", left: "左侧", back: "背面", right: "右侧" }, requires: { rig: "需要先对该模型自动绑骨，套用动画只能接在本画布绑骨产生的节点后面", segment: "需要先对该模型分割部件，网格补全只能接在本画布分割产生的节点后面" }, notRiggable: "Tripo 判定该模型无法绑骨（识别为 {{type}}），已中止以免消耗 credits", noAnimationForRigType: "该骨架类型暂无可用预设动画", rigModelHint: "推荐使用 {{model}}，预设动画更多", downloadConverted: "下载 {{ext}}", convertDone: "已转换为 {{ext}}，可从「3D 操作」菜单下载", convertedMissing: "转换结果已丢失，请重新转换", splitDone: "已拆出 {{count}} 个部件节点", splitNoParts: "这个模型没有可识别的部件名，无法拆件；请改用 v2.0 分割模型重试" }, blenderBridge: { title: "导入到 Blender", label: "导入 Blender", importing: "正在导入 {{count}} 个模型到 Blender…", imported: "已导入 {{count}} 个模型到 Blender", none: "选中的节点里没有可导入的 3D 模型", missing: "找不到「{{name}}」的模型文件", unsupported: "「{{name}}」的格式（{{format}}）Blender 插件不支持，请先转换成 GLB / FBX / OBJ / STL / USDZ", busy: "正在导入到 Blender，请稍候", connectFailed: "连不上 Blender（ws://127.0.0.1:60600）。请先在本机 Blender 里启用 Tripo Bridge 插件", handshakeFailed: "与 Blender 握手失败，请确认 Tripo Bridge 插件已启用", timeout: "等待 Blender 响应超时，请确认插件仍在运行", transferFailed: "「{{name}}」传输失败", importFailed: "「{{name}}」导入失败{{reason}}", mixedHttps: "当前页面是 HTTPS，浏览器不允许连接本机 ws://。请用本机 http://localhost:3000 打开画布后再导入" }, bambuStudio: { title: "用 Bambu Studio 打开", label: "打开 Bambu Studio", opening: "正在唤起 Bambu Studio（{{count}} 个）…", opened: "已唤起 Bambu Studio（{{count}} 个）", converting: "正在把 {{count}} 个模型转成 3MF…", busy: "「{{name}}」正在处理中，请稍候再打开 Bambu Studio", configRequired: "请先配置 Tripo 渠道，才能把模型转成 3MF", none: "选中的节点里没有可打开的 3D 模型", missing: "找不到「{{name}}」的 3MF 文件", needConvert: "「{{name}}」还没有 3MF，请先在「3D 操作」里转换成 3MF", agentRequired: "本地 3MF 需要 Canvas Agent 临时托管给 Bambu Studio 下载。请先连接本机 Agent，或重新转换一次以使用 Tripo 的下载地址", openFailed: "无法唤起 Bambu Studio，请确认本机已安装并注册了 bambustudio:// 协议" }, videoFrames: { first: "截取首帧", last: "截取尾帧", current: "截取当前帧", firstTitle: "{{name}} 首帧", lastTitle: "{{name}} 尾帧", currentTitle: "{{name}} 当前帧", captured: "已生成图片节点", failed: "无法截取该画面，请重试" },
        sidePanel: {
            canvas: "画布", assets: "资产", prompts: "提示词库", history: "历史", resize: "调整左侧面板宽度", elements: "画布元素", select: "选择", searchNodes: "搜索节点", focusNode: "定位到节点", preview: "放大预览", noNodes: "画布暂无节点", clearAll: "取消全选", selected: "已选 {{count}}", exporting: "正在导出选中元素…", exportName: "画布元素-{{count}}个", exported: "已导出 {{count}} 个元素", exportFailed: "导出失败，请重试",
            ...
            filter: { image: "图片", video: "视频", text: "文本", audio: "音频", model3d: "3D", config: "配置", group: "分组" },
            historyRuns: "生成记录", searchHistory: "搜索提示词、模型或任务 ID", noHistory: "暂无生成记录", copyTaskId: "复制任务 ID", taskIdCopied: "任务 ID 已复制", noTaskId: "该记录没有任务 ID", historyNodeGone: "对应节点已删除", removeHistoryTitle: "删除这条记录？", clearHistory: "清空", clearHistoryTitle: "清空本画布的生成记录？", historyCleared: "生成记录已清空", historyMetaOnly: "只记录任务 ID、提示词和结果路径；节点删除后文件可能被回收，记录仍可用于查任务或重新生成。",
            historyKind: { image: "图片", video: "视频", audio: "音频", model3d: "3D", multiview: "多视图", model3dOp: "3D 操作" },
        },
        assetPicker: { title: "选择资产", insert: "插入", search: "搜索资产", empty: "没有资产" },
        imageTools: { copyPrompt: "复制提示词", copyPromptTitle: "复制生成该图片的提示词", reversePrompt: "反推提示词", reversePromptTitle: "创建反推提示词的文本和配置节点", replace: "替换图片", locked: "锁比例", free: "自由比例", lockTitle: "切换为等比缩放", freeTitle: "切换为自由比例", mask: "局部编辑", maskTitle: "添加蒙版遮罩后局部修改", crop: "裁剪", cropTitle: "裁剪并生成新节点", split: "切图", splitTitle: "按行列切分图片", decompose: "拆件", decomposeTitle: "按画面内容拆成单个物件并逐个生成", multiview: "多视图", multiviewTitle: "用 Tripo 生成四视图（消耗 credits）", builtinMultiview: "四视图", builtinMultiviewTitle: "用当前图片模型分别生成正/左/后/右四个视角（不走 Tripo）", upscale: "放大", upscaleTitle: "放大图片分辨率", superResolve: "超分", superResolveTitle: "AI 超分", angle: "多角度", angleTitle: "生成角度", view: "查看大图", viewTitle: "查看图片详情", more: "更多", configure: "配置快捷工具", customize: "自定义工具栏", showLabels: "显示按钮文字", description: "选择你想在图片节点编辑栏中使用的快捷工具。", preview: "节点预览", imageNode: "图片节点", quickTools: "快捷工具" },
        nodeToolbar: { noPrompt: "暂无可复制的提示词", infoTitle: "查看节点信息", info: "信息", removeTitle: "移除节点", retryTitle: "重新生成", queryVideoTask: "获取任务状态", queryVideoTaskTitle: "使用任务 ID 查询视频生成状态", videoTaskId: "任务 ID", saveAsset: "存资产", downloadAudio: "下载音频", downloadModel3d: "下载 3D 模型", downloadVideo: "下载视频", downloadImage: "下载图片", editTextTitle: "编辑文本", editText: "编辑文字", decreaseFont: "减小字号", increaseFont: "增大字号", zoomOut: "缩小", zoomIn: "放大", uploadImage: "上传图片", replaceVideo: "替换视频", uploadVideo: "上传视频", replaceAudio: "替换音频", uploadAudio: "上传音频", nodeInfo: "节点信息", name: "名称", type: "类型", size: "尺寸", position: "位置", status: "状态", imageGroup: "图片组", imageSize: "图片大小", group: "打组", groupTitle: "用组节点包住选中节点", ungroup: "解散组", ungroupTitle: "取消节点分组" },
        configNode: { title: "生成配置", image: "生图", text: "文本", video: "视频", audio: "音频", model3d: "3D", prompt: "提示词", references: "参考图", videoReferences: "参考视频", audioReferences: "参考音频", items: "{{count}} 个", images: "{{count}} 张", compose: "组装提示词", stop: "停止", generate: "开始生成", parameterSource: "作为参数节点，供下游 {{count}} 个节点使用" },
        parameters: { title: "参数", inherited: "参数来自上游配置节点「{{title}}」", focusSource: "定位到配置节点", disconnectSource: "断开配置节点" },
        multiview: { assignTitle: "指定这张图的视角", unassigned: "未指定" },
        projectPage: { multiviewGroupTitle: "四视图", model3dMultiviewTrimmed: "多视图最多支持 {{count}} 张图片，已使用前 {{count}} 张", multiviewPartialFailure: "有 {{count}} 个视角生成失败，其余视角已保留", multiviewRateLimited: "触发生成并发上限，{{seconds}} 秒后重试这个视角", decomposeGroupTitle: "场景组件", decomposePartialFailure: "有 {{count}} 个组件生成失败，其余组件已保留", decomposeRateLimited: "触发生成并发上限，{{seconds}} 秒后重试这个组件",
        ...
        decompose: {
            title: "拆件",
            description: "先识别画面中可以单独取出的物件，确认清单后逐个生成带透明背景的单体图。",
            analyzing: "正在识别画面中的组件…",
            analyzeFailed: "识别组件失败",
            configMissing: "请先配置文本模型",
            components: "组件（{{count}}）",
            reanalyze: "重新识别",
            add: "添加一项",
            remove: "删除这一项",
            empty: "没有识别到可单独取出的物件，可以手动添加或重新识别。",
            namePlaceholder: "物件名称",
            truncated: "最多识别 {{count}} 个组件，已按重要性截取",
            opaqueBackground: "当前图片模型不支持透明背景，拆出的组件会是不透明底。",
            generate: "生成 {{count}} 个组件",
        },
        editors: {
        ...
        promptPanel: { model3d: "描述要生成的 3D 模型", model3dSingleImageHint: "已连接 1 张图片，将进行图生 3D，无需提示词", model3dMultiviewHint: "已连接 {{count}} 张图片，将进行多视图生 3D，无需提示词。单张图接到左侧对应视角接点，打组的图在组内标记正/左/后/右；未指定的按连线顺序补上剩下的视图", model3dMultiviewTrimmedHint: "已连接 {{count}} 张图片，多视图最多只用前 {{max}} 张，无需提示词。单张图接到左侧对应视角接点，打组的图在组内标记正/左/后/右；未指定的按连线顺序补上剩下的视图", video: "描述要生成的视频内容", audio: "描述要生成的音频内容", image: "描述要生成的图片内容", text: "请输入你想要生成的文本内容", editImage: "请输入你想要把这张图修改成什么", editText: "请输入你想要将本段文本修改成什么", expandEditor: "放大编辑", editorTitle: "编辑提示词", stopGeneration: "停止生成", generate: "生成", stop: "停止" },
    ...
    config: {
        ...
        proxy: {
            ...
            hostedTitle: "转发代理",
            hostedDescription: "当前部署已内置同源转发代理。生成请求会经本站点转到上游接口，用来绕开浏览器跨域限制。API Key 仍保存在你的浏览器里，请求时随头转发，站点不会存储。",
            hostedAddressDescription: "由部署注入，无需本机再跑代理命令。",
            hostedChannelHint: "渠道仍填写真实地址。自定义渠道的域名若不在白名单里会被拒绝转发，需要部署方把该域名加进 PROXY_ALLOW_HOSTS。",
            test: "测试连接",
            available: "代理连接正常（{{proxy}}）",
            missingUrl: "请先填写代理地址。",
            unreachable: "无法连接代理，请确认地址填写正确。",
        },
        localStorage: {
        ...
        channels: {
            ...
            missingUrl: "未填写接口地址",
            keepOne: "至少保留一个渠道",
            defaultName: "默认渠道", tripoCnName: "Tripo 国内站", tripoGlobalName: "Tripo 海外站",
            newName: "新渠道",
        ...
        channelEditor: {
            tripoRegion: "Tripo 区域", tripoRegionHint: "国内站和海外站是两套独立账号，API Key 不通用；也可在下方直接填写自定义地址。", tripoRegions: { cn: "国内站 (.com)", global: "海外站 (.ai)" },
            title: "编辑渠道",
            name: "渠道名称",
            protocol: "协议",
            baseUrl: "接口地址",
